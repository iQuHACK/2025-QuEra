...
def circuit2():
    ...
    state = move.LocalXY(atom_state=state, x_exponent=-0.5*pi, axis_phase_exponent=-0.5*pi, indices=[0])
    ...
    state = move.LocalRz(atom_state=state, phi=5/4*pi,indices=[3])
    state = move.LocalXY(atom_state=state, x_exponent=0.5*pi, axis_phase_exponent=-0.5*pi, indices=[3])
    ...
    state = move.LocalXY(atom_state=state, x_exponent=0.5*pi, axis_phase_exponent=-0.5*pi, indices=[2])
    state = move.LocalRz(atom_state=state, phi=7/8*pi,indices=[2])
    state = move.LocalXY(atom_state=state, x_exponent=0.5*pi, axis_phase_exponent=-0.5*pi, indices=[2])
    state = move.GlobalCZ(atom_state=state)
    state = move.GlobalRz(atom_state=state,phi=pi)
    state = move.LocalRz(atom_state=state, phi=pi,indices=[1])
    state = move.GlobalXY(atom_state=state, x_exponent=0.5*pi, axis_phase_exponent=-0.5*pi)
    state = move.LocalXY(atom_state=state, x_exponent=-0.5*pi, axis_phase_exponent=-0.5*pi, indices=[1])
    state = move.LocalRz(atom_state=state, phi=pi/4,indices=[2])
    move.Execute(state)
...
make_gif(scorer, "circuit2.alt.gif")