...
def circuit1():
    ...
    state = move.LocalXY(atom_state=state,x_exponent=pi/2,axis_phase_exponent=-pi/4,indices=[1])
    state = move.LocalXY(atom_state=state,x_exponent=-3/4*pi,axis_phase_exponent=0.,indices=[2])
    state = move.GlobalCZ(atom_state=state)
    state = move.LocalRz(atom_state=state,phi=-0.75*pi,indices=[0])
    state = move.LocalXY(atom_state=state,x_exponent=pi/2,axis_phase_exponent=3/4*pi,indices=[1])
    state = move.LocalRz(atom_state=state,phi=pi/4,indices=[1])
    state.gate[[3]] = move.Move(state.gate[[1]])
    state = move.GlobalCZ(atom_state=state)
    state = move.LocalXY(atom_state=state,x_exponent=pi/2,axis_phase_exponent=-3/4*pi,indices=[3])
    state = move.LocalRz(atom_state=state,phi=3/4*pi,indices=[3])
    state = move.LocalXY(atom_state=state,x_exponent=pi/4,axis_phase_exponent=0.,indices=[2])
    ...
    state = move.LocalXY(atom_state=state,x_exponent=pi/2,axis_phase_exponent=pi,indices=[2])
    state.gate[[1]] = move.Move(state.gate[[3]])
    state = move.GlobalCZ(atom_state=state)
    state = move.LocalXY(atom_state=state,x_exponent=pi/2,axis_phase_exponent=-pi/2,indices=[1])
    state.gate[[3]] = move.Move(state.gate[[1]])
